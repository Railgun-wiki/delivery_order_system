...
* Google AI Gemini Pro (对话分析需求和了解技术)
...
**涉及流程**:
1.  **需求分析与原型设计**：开始前与网页 Gemini 对话补充系统的需求，了解适用的技术和优化提供给 Copilot 的提示词 (不涉及编程)。
2.  **原型架构设计和实现**：向 VS Code 中 Copilot 工具提交优化后的提示词，使其完成第一版系统架构的设计和实现。
3.  **架构迭代与重构**：通过对话探讨持久化与扩展性瓶颈，平滑迁移至 SQLite 数据库方案。
4.  **核心业务编码与防御性编程**：梳理状态机流转跃迁，加入大量的边界拦截和异常防护。
5.  **测试驱动与分离**：将测试逻辑从主程序剥离为 `test/test_delivery_order_system.py` 自动化套件。
6.  **辅助工程化与交付**：编写项目 README、自动化聊天导出脱敏工具 `tool/convert_chat.py`、自动化打包工具`tool/package.py`，整理项目文档体系。

## 9. AI Coding 反思
在系统技术选型迭代、工程文件重构、业务逻辑debug的过程中，总结了以下AI Coding开发经验与反思：

### 1. AI Coding 工作流

* **提示词设计** 明确约束，先定接口，再定结构。必须在提示词中明确指出：
  - **功能边界：** 需要实现哪几个具体方法。
  - **核心数据结构：** 根据个人对数据结构的理解确定使用的结构。
  - **禁止规则：** 提前封杀低效或错误的实现。
  - **边界与测试：** 要求覆盖空队列、无记录等极端情况，并提供测试用例。
* **人工审查** 检查 AI 生成代码的可用性和是否符合业务逻辑：
  - **语义审查：** 业务逻辑如 FIFO 的调度原则是否正确。
  - **复杂度审查：** 是否达到处理该逻辑的最优复杂度。
  - **边界条件审查：** 极端情况下是否会崩溃，错误情况是否被合理拦截。
  - **状态一致性审查：** 撤销操作后，撤销栈、完成历史、当前订单这三个状态是否同步闭合，是否产生了幽灵数据。

### 2. AI Coding 反思

总而言之，我认为在AI编程时需要开发者更多的宏观架构设计思考和防守性的引导和审查。

1. **AI Coding中对人类对架构设计与迭代把控**：

   目前的大模型在处理长逻辑链和复杂状态同步时，依然容易出现幻觉或遗漏。面对复杂的综合系统中，可能同时牵扯到多个的数据结构变化的逻辑，很容易顾此失彼，需要开发者提示。同时AI难以跳出既有代码框架去构思更优的重构方案，这要求开发者持续介入，主导全局架构层面的思考与方向指引。

2. **编程时测试样例的一致性**：
   在技术迭代时，重构的过程中始终保持外部 API 封装不变。底层的存储从内存指针变成了 SQL 交互和类、系统从单一模块拆分为了 `src` 和 `test` 子包，但是 `DeliveryOrderSystem` 暴露的接口和调用方法维持不变。这使得 `test_delivery_order_system.py` 中的测试用例能够维持一致，为内部逻辑变动的正确性提供了直接的验证依据。

3. **负面约束提示重要性**：
    如“The forward logic is only about 10% of your code,everything else is 90%.”，在通过大模型生成业务代码时，如果仅给出需求的指令，往往只能得到10%的过度简单且有隐患的代码。当提示词被强化为包含负面的指令，如明确要求程序处理撤销操作与现有排队状态的互斥逻辑，或拦截一切针对不存在订单的非法请求，能够有效驱动大模型主动推演系统边界，覆盖更多潜在的异常场景。